...
it('one to many relationship saved corectly in the relationship json field as well as migrated correctly', function () {
     ...
     */
    $invoice = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'Invoice',
        ])
        ->call('create')
        ...
        ->fillForm([
            ...
            'relationships' => [
               [
                    ...
                    'relationship_b_table' => CollectionHelper::uuidToTableName($invoice->record->id)
               ]
            ]
        ])
        ->call('create')
    ...
    $invoiceCollection = Collection::where('name', '=', 'Invoice')->first();

    $clientTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($clientCollection->id) . ')');
    $invoiceTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($invoiceCollection->id) . ')');
    ...
    expect($invoiceTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $invoiceTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($invoiceCollection->id) . ')');

    expect(count($clientTableForeignKeyList))->toEqual(0);
    expect(count($invoiceTableForeignKeyList))->toEqual(1);
    
    expect($invoiceTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($invoiceTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($clientRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($invoiceCollection->id));

    expect($invoiceRelationships[0]['id'])->toBeUuid();
    // Profile belongs to client
    expect($invoiceRelationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($invoiceCollection->id));
    expect($invoiceRelationships[0]['relationship_type'])->toEqual('belongsTo');
    expect($invoiceRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});

// Update
it('one to many relationship update by adding a new relationship on a collection that already has one thus json fields update accordingly and migrations run correctly too', function () {
    $invoice = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'Invoice',
        ])
    ...
    $newInvoice = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'NewInvoice',
        ])
        ->call('create')
        ...
        ->fillForm([
            'relationships' => [
                ...
                [
                    ...
                    'relationship_a_table' => null,
                    'relationship_type' => 'hasMany',
                    'relationship_b_table' => CollectionHelper::uuidToTableName($newInvoice->record->id)
                ],
            ],
        ])
        ->call('save')
    ...
    $clientCollection = Collection::where('name', '=', 'Client')->first();
    $invoiceCollection = Collection::where('name', '=', 'Invoice')->first();
    $newInvoiceCollection = Collection::where('name', '=', 'NewInvoice')->first();
    /**
    ...
    $updatedInvoiceTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($invoiceCollection->id) . ')');
    $updatedNewInvoiceTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($newInvoiceCollection->id) . ')');
    ...
    expect($updatedNewInvoiceTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $updatedInvoiceTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($invoiceCollection->id) . ')');
    $updatedNewInvoiceTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($newInvoiceCollection->id) . ')');
    ...
    expect($updatedInvoiceTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedInvoiceTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($updatedNewInvoiceTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedNewInvoiceTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($clientRelationships[1]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($newInvoiceCollection->id));
    ...
    expect($newInvoiceRelationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($newInvoiceCollection->id));
    expect($newInvoiceRelationships[0]['relationship_type'])->toEqual('belongsTo');
    expect($newInvoiceRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});
...
it('one to many relationship deleted thus json fields update accordingly and migrations run correctly too', function () {
    ...
    $controlInvoice = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'ControlInvoice',
        ])
        ->call('create')
        ...
        ->fillForm([
            ...
            'relationships' => [
                ...
                [
                    ...
                    'relationship_a_table' => null,
                    'relationship_type' => 'hasOne',
                    'relationship_b_table' => CollectionHelper::uuidToTableName($controlInvoice->record->id)
                ],
            ]
        ])
        ->call('create')
    ...
    $clientCollection = Collection::where('name', '=', 'Client')->first();
    $invoiceCollection = Collection::where('name', '=', 'Invoice')->first();
    $controlInvoiceCollection = Collection::where('name', '=', 'ControlInvoice')->first();
    ...
    $updatedInvoiceCollection = Collection::where('name', '=', 'Invoice')->first();
    $updatedControlInvoiceCollection = Collection::where('name', '=', 'ControlInvoice')->first();
    ...
    $updatedInvoiceTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($updatedInvoiceCollection->id) . ')');
    $updatedControlInvoiceTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($updatedControlInvoiceCollection->id) . ')');
    ...
    expect($updatedControlInvoiceTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $updatedInvoiceTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($updatedInvoiceCollection->id) . ')');
    $updatedControlInvoiceTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($updatedControlInvoiceCollection->id) . ')');

    expect(count($updatedClientTableForeignKeyList))->toEqual(0);
    expect(count($updatedInvoiceTableForeignKeyList))->toEqual(0);
    expect(count($updatedControlInvoiceTableForeignKeyList))->toEqual(1);

    expect($updatedControlInvoiceTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedControlInvoiceTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($updatedClientCollection->relationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($controlInvoiceCollection->id));
    ...
    expect($updatedControlInvoiceCollection->relationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($controlInvoiceCollection->id));
    ...
    expect($updatedControlInvoiceCollection->relationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});