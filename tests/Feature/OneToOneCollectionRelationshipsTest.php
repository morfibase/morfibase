...
it('one to one relationship saved corectly in the relationship json field as well as migrated correctly', function () {
     ...
     */
    $profile = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'Profile',
        ])
        ->call('create')
    ...
    $client = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'Client',
            'relationships' => [
               [
                    ...
                    'relationship_b_table' => CollectionHelper::uuidToTableName($profile->record->id)
               ]
            ]
        ])
        ->call('create')
    ...
    $clientCollection = Collection::where('name', '=', 'Client')->first();
    $profileCollection = Collection::where('name', '=', 'Profile')->first();

    $clientTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($clientCollection->id) . ')');
    $profileTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($profileCollection->id) . ')');
    ...
    expect($profileTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $clientTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($clientCollection->id) . ')');
    $profileTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($profileCollection->id) . ')');

    expect(count($clientTableForeignKeyList))->toEqual(0);
    expect(count($profileTableForeignKeyList))->toEqual(1);
    
    expect($profileTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($profileTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($clientRelationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($clientRelationships[0]['relationship_type'])->toEqual('hasOne');
    expect($clientRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($profileCollection->id));

    expect($profileRelationships[0]['id'])->toBeUuid();
    // Profile belongs to client
    expect($profileRelationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($profileCollection->id));
    expect($profileRelationships[0]['relationship_type'])->toEqual('belongsTo');
    expect($profileRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});

// Update
it('one to one relationship update by adding a new relationship on a collection that already has one thus json fields update accordingly and migrations run correctly too', function () {
    $profile = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'Profile',
        ])
    ...
    $newProfile = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'NewProfile',
        ])
        ->call('create')
        ...
        ->fillForm([
            'relationships' => [
                ...
                [
                    ...
                    'relationship_a_table' => null,
                    'relationship_type' => 'hasOne',
                    'relationship_b_table' => CollectionHelper::uuidToTableName($newProfile->record->id)
                ],
            ],
        ])
        ->call('save')
    ...
    $clientCollection = Collection::where('name', '=', 'Client')->first();
    $profileCollection = Collection::where('name', '=', 'Profile')->first();
    $newProfileCollection = Collection::where('name', '=', 'NewProfile')->first();
    /**
     * Check if the migrations ran correctly
     */

    $updatedClientTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($clientCollection->id) . ')');
    $updatedProfileTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($profileCollection->id) . ')');
    $updatedNewProfileTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($newProfileCollection->id) . ')');
    ...
    expect($updatedProfileTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($updatedNewProfileTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $updatedClientTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($clientCollection->id) . ')');
    $updatedProfileTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($profileCollection->id) . ')');
    $updatedNewProfileTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($newProfileCollection->id) . ')');
    ...
    expect($updatedProfileTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedProfileTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($updatedNewProfileTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedNewProfileTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    // Client has one profile
    expect($clientRelationships[1]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($clientRelationships[1]['relationship_type'])->toEqual('hasOne');
    expect($clientRelationships[1]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($newProfileCollection->id));
    ...
    expect($newProfileRelationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($newProfileCollection->id));
    expect($newProfileRelationships[0]['relationship_type'])->toEqual('belongsTo');
    expect($newProfileRelationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});
...
it('one to one relationship deleted thus json fields update accordingly and migrations run correctly too', function () {
    ...
    $controlProfile = livewire(CreateCollection::class)
        ->fillForm([
            'name' => 'ControlProfile',
        ])
        ->call('create')
        ...
        ->fillForm([
            ...
            'relationships' => [
                ...
                [
                    ...
                    'relationship_a_table' => null,
                    'relationship_type' => 'hasOne',
                    'relationship_b_table' => CollectionHelper::uuidToTableName($controlProfile->record->id)
                ],
            ]
        ])
        ->call('create')
    ...
    $clientCollection = Collection::where('name', '=', 'Client')->first();
    $profileCollection = Collection::where('name', '=', 'Profile')->first();
    $controlProfileCollection = Collection::where('name', '=', 'ControlProfile')->first();

    // Navigate to the "Relationships" tab and delete the relationship then save
    $page = visit(CollectionResource::getUrl('edit', ['record' => $clientCollection]));
    ...
    $updatedClientCollection = Collection::where('name', '=', 'Client')->first();
    $updatedProfileCollection = Collection::where('name', '=', 'Profile')->first();
    $updatedControlProfileCollection = Collection::where('name', '=', 'ControlProfile')->first();

    /**
     * Check if the migrations ran correctly
     */

    $updatedClientTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($updatedClientCollection->id) . ')');
    $updatedProfileTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($updatedProfileCollection->id) . ')');
    $updatedControlProfileTableSchema = DB::select('PRAGMA table_info(' . CollectionHelper::uuidToTableName($updatedControlProfileCollection->id) . ')');
    ...
    expect($updatedControlProfileTableSchema[1]->name)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    $updatedClientTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($updatedClientCollection->id) . ')');
    $updatedProfileTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($updatedProfileCollection->id) . ')');
    $updatedControlProfileTableForeignKeyList = DB::select('PRAGMA foreign_key_list(' . CollectionHelper::uuidToTableName($updatedControlProfileCollection->id) . ')');

    expect(count($updatedClientTableForeignKeyList))->toEqual(0);
    expect(count($updatedProfileTableForeignKeyList))->toEqual(0);
    expect(count($updatedControlProfileTableForeignKeyList))->toEqual(1);

    expect($updatedControlProfileTableForeignKeyList[0]->table)->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    expect($updatedControlProfileTableForeignKeyList[0]->from)->toEqual(CollectionHelper::uuidToForeignKeyName($clientCollection->id));
    ...
    expect($updatedClientCollection->relationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
    ...
    expect($updatedClientCollection->relationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($controlProfileCollection->id));
    ...
    expect($updatedControlProfileCollection->relationships[0]['relationship_a_table'])->toEqual(CollectionHelper::uuidToTableName($controlProfileCollection->id));
    ...
    expect($updatedControlProfileCollection->relationships[0]['relationship_b_table'])->toEqual(CollectionHelper::uuidToTableName($clientCollection->id));
});